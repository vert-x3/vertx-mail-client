...
* `subject` String representing the subject of the mail (must be single line)
...
* `maxMailsPerConnection`, long, the max emails count per connection before it gets closed
