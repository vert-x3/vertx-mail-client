 ...
 *
 * This copies from https://github.com/apache/httpcomponents-client/blob/5.0.3-RC1/httpclient5/src/main/java/org/apache/hc/client5/http/impl/auth/NTLMEngineImpl.java
 * with modifications on Base64 to use JDK implementations.
 *
 */
final class NTLMEngineImpl {
    ...
    private static byte[] lmHash(final char[] password) throws NTLMEngineException {
        try {
            ...
            final byte[] oemPassword = Buffer.buffer(new String(tmp), UNICODE_LITTLE_UNMARKED.name()).getBytes();
    ...
    private static byte[] ntlmHash(final char[] password) throws NTLMEngineException {
        ...
        final byte[] unicodePassword = Buffer.buffer(new String(password), UNICODE_LITTLE_UNMARKED.name()).getBytes();
        final MD4 md4 = new MD4();
