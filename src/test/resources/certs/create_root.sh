...
openssl req -x509 -new -nodes -key rootCA.key -sha256 -days 7300 -out rootCA.pem -config openssl.cnf -extensions v3_ca -batch
keytool -importcert -noprompt -storetype jks -storepass password -keystore client.jks -file rootCA.pem -alias rootca
...
openssl x509 -req -in server.csr -CA rootCA.pem -CAkey rootCA.key -CAcreateserial -out server.crt -days 7300 -sha256 -extensions req_ext -extfile openssl.cnf
openssl pkcs12 -export -passout pass:password -inkey server.key -in server.crt -certfile rootCA.pem -out server.p12
...
openssl req -new -key server2.key -out server2.csr -config openssl.cnf -batch
openssl x509 -req -in server2.csr -CA rootCA.pem -CAkey rootCA.key -CAcreateserial -out server2.crt -days 7300 -sha256 -extensions req_ext -extfile openssl.cnf
openssl pkcs12 -export -passout pass:password -inkey server2.key -in server2.crt -certfile rootCA.pem -out server2.p12
